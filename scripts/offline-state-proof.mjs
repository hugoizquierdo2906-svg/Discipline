// OfflineState proof — the no-connectivity indicator. Captures (desktop/tablet/
// mobile) plus programmatic assertions for rendering (title/description),
// the warning-tinted icon, the Retry action present/absent, sizes, alignment
...
async function newPage(width, height) {
  ...
  await page.goto(`${BASE}/dev/offline-state`, { waitUntil: 'networkidle' })
...
// The OfflineState root is the parent of its heading (icon/title/description/
...
for (const [w, h, suffix] of [
  ...
  await page.screenshot({
    path: `${OUT}/offline-state${suffix}.png`,
    fullPage: true,
  })
  console.log(`screenshot: ${OUT}/offline-state${suffix}.png`)
  await context.close()
}
...
{
  const { context, page } = await newPage(1280, 900)
  const r = root(page, 'offline-basic')
  ...
  if (iconColor === bodyColor)
    issues.push(
      `[assert] offline icon should be tinted (text-warning), not the body color; both are ${iconColor}`,
    )
  await context.close()
}
...
{
  const { context, page } = await newPage(1280, 900)
  const s = section(page, 'offline-retry')
...
{
  const { context, page } = await newPage(1280, 900)
  const s = section(page, 'offline-no-action')
...
{
  const { context, page } = await newPage(1280, 900)
  const centerAlign = await root(page, 'offline-center').evaluate(
    (e) => getComputedStyle(e).textAlign,
  )
  if (centerAlign !== 'center')
    issues.push(
      `[assert] centered OfflineState text-align expected center, got ${centerAlign}`,
    )
  const inlineAlign = await root(page, 'offline-inline').evaluate(
    (e) => getComputedStyle(e).textAlign,
  )
  if (inlineAlign !== 'start' && inlineAlign !== 'left')
    issues.push(
      `[assert] inline OfflineState text-align expected start/left, got ${inlineAlign}`,
    )
  await context.close()
}
...
// Responsive: centered content stays horizontally centered at any viewport.
{
  for (const w of [1280, 390]) {
    ...
    const off = await root(page, 'offline-center').evaluate((e) => {
    ...
    if (off > 2)
      issues.push(
        `[assert] centered OfflineState should stay centered at ${w}px, icon off by ${off}px`,
      )
    await context.close()
  }
}
...
{
  const { context, page } = await newPage(1280, 900)
  const scene = section(page, 'offline-rtl')
  ...
  if (dir !== 'rtl')
    issues.push(
      `[assert] RTL OfflineState computed direction expected rtl, got ${dir}`,
    )
  if ((await scene.locator('button').count()) !== 1)
    issues.push('[assert] RTL OfflineState retry button missing')
  await scene.scrollIntoViewIfNeeded()
  await page.waitForTimeout(150)
  await scene.screenshot({ path: `${OUT}/offline-state-rtl.png` })
  console.log(`screenshot: ${OUT}/offline-state-rtl.png`)
  await context.close()
}

// Static: no animation on the OfflineState root (ignore the composed Button).
{
  const { context, page } = await newPage(1280, 900)
  const animated = await root(page, 'offline-retry').evaluate((e) => {
  ...
  if (animated)
    issues.push(
      `[assert] OfflineState should be static (no animation): ${animated}`,
    )
  await context.close()
}
