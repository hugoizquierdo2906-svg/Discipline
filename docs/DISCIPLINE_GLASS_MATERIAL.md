...
Status: VALIDATED (material spec) · Governed by DISCIPLINE_CONSTITUTION.md
...
## 6b. Light Behaviour (the material's physical law)

The glass is **one optical material**, not a collection of CSS effects. It always
obeys the same physical laws of light. These rules are absolute and apply to every
glass component without exception:

- **One light direction for the entire interface.** A single implied light source
  (soft, upper-left, slightly frontal) is shared by every component.
- **One reflection logic everywhere.** All components use exactly the same
  reflection model — edge light, inner highlight, inner shadow and specular are
  derived from that one light, never authored per component.
- **Reflections are never arbitrary.** Every highlight exists because the shared
  light produced it. No decorative or hand-placed shine.
- **The pastel-violet reflection is always generated by this same light.** The
  violet capture is not an added color; it is what this one light leaves inside
  the glass on surfaces that hold it.
- **It appears only where the surface naturally catches that light** — the
  light-facing top/upper-left rim — never on faces turned away from the light.
- **It never fully surrounds the component.** No complete violet ring or outline.
- **It is never perfectly centered.**
- **It is slightly asymmetric**, so it looks natural rather than constructed.
- **It shifts slightly by state** (rest / hover / active) as the glass moves
  toward or away from the light — but it **never changes color**.
- **The material always appears to have real thickness** — never a flat pane.
- **Components must feel cut from the same block of glass** — identical material,
  only the thickness tier differs.

---

## 6c. Coherence principle

If every piece of content were removed from the interface and only the glass
components remained, it must be **immediately obvious that they all belong to the
same material**. Same light, same reflection logic, same thickness family, same
violet behavior. Any component that would look like a different material under
this test is incorrect.

---

## 6d. Material validation rule

Before a component is considered finished, it is **compared to the Primary
reference button**. If the material looks different from that reference, the
component is **incorrect — even if its CSS is valid**. Material fidelity, not code
validity, is the acceptance criterion for the glass.
