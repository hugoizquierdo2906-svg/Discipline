...
| `Switch` | Built (not frozen) | **Rebuild on the frozen micro-control foundation 2026-07-02**: GlassSurface → Micro Surface → micro-control → Switch. EXACTLY the frozen Checkbox/Radio language (token border · accent active fill · motion · dim · invalid rim · global focus ring); only the geometry changes — a 44×24 track and the sliding 18px thumb, which IS the dark glyph (✓/● philosophy in motion; new additive `microControlThumbClass`). Objective bug fixed: the old `h-6 w-11` track was 32×96px on the DISCIPLINE scale (not 24×44) and `translate-x-5` (24px) never reached the right edge — geometry is now explicit px. switch.tsx grep: zero material/shadow/blur/focus/transition strings. On Radix Switch (Space/Enter, Tab, ARIA switch). API: label · description · error · helperText · invalid · readOnly (focusable, never toggles) · required · labelPosition · checked/defaultChecked/onCheckedChange. Backward compatible. Proof: `/dev/switch`. |
